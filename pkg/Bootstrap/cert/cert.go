...
func Generatecert() ([]byte, []byte, []byte) {
	err := os.MkdirAll("/var/lib/kubernetes", 0750)
	...
	os.WriteFile("ca.pem", cacert, 0644)
	...
	KubeCtrlManagercert, KubeCtrlManagerkey := KubeControllerManagercertsetup(caBytes, caPrivKey)
	Kubeapiservercertsetup(caBytes, caPrivKey)
	ServiceAccountcertsetup(caBytes, caPrivKey)
	MoveFile("ca.pem", "/var/lib/kubernetes/ca.pem")
	MoveFile("ca-key.pem", "/var/lib/kubernetes/ca-key.pem")
	return cacert, KubeCtrlManagercert, KubeCtrlManagerkey
}
...
func admincertsetup(ca []byte, caPrivKey *rsa.PrivateKey) {
	...
	MoveFile("admin.pem", "/var/lib/kubernetes/admin.pem")
	MoveFile("admin-key.pem", "/var/lib/kubernetes/admin-key.pem")
}
func KubeControllerManagercertsetup(ca []byte, caPrivKey *rsa.PrivateKey) ([]byte, []byte) {
	now := time.Now()
	caCertificate, err := x509.ParseCertificate(ca)
	if err != nil {
		return nil, nil
	...
	KubeCtrlManagercert := pem.EncodeToMemory(&pem.Block{Type: "CERTIFICATE", Bytes: KubeControllerManagercertBytes})
	os.WriteFile("kube-controller-manager.pem", KubeCtrlManagercert, 0644)
	//if err != nil {
	...
	KubeCtrlManagerkey := pem.EncodeToMemory(&pem.Block{Type: "PRIVATE KEY", Bytes: KubeControllerManagercertprivBytes})
	os.WriteFile("kube-controller-manager-key.pem", KubeCtrlManagerkey, 0644)
	...
	MoveFile("kube-controller-manager.pem", "/var/lib/kubernetes/kube-controller-manager.pem")
	MoveFile("kube-controller-manager-key.pem", "/var/lib/kubernetes/kube-controller-manager-key.pem")

	return KubeCtrlManagercert, KubeCtrlManagerkey
}
...
func Kubeapiservercertsetup(ca []byte, caPrivKey *rsa.PrivateKey) {
	...
	MoveFile("kubernetes.pem", "/var/lib/kubernetes/kubernetes.pem")
	MoveFile("kubernetes-key.pem", "/var/lib/kubernetes/kubernetes-key.pem")
}
func ServiceAccountcertsetup(ca []byte, caPrivKey *rsa.PrivateKey) {
	...
	MoveFile("service-account.pem", "/var/lib/kubernetes/service-account.pem")
	MoveFile("service-account-key.pem", "/var/lib/kubernetes/service-account-key.pem")
}
